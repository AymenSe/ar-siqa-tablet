...
router = APIRouter(prefix="/session-images", tags=["Session Images"])

@router.post("/", response_model=schemas.SessionImageOut)
def create_session_image(si: schemas.SessionImageCreate, db: Session = Depends(get_db)):
    # Validate session and image exist
    session = db.query(models.Session).get(si.session_id)
    image = db.query(models.Image).get(si.image_id)
    if not session or not image:
        raise HTTPException(status_code=404, detail="Session or Image not found")
    
    new_si = models.SessionImage(**si.dict())
    db.add(new_si)
    db.commit()
    db.refresh(new_si)
    return new_si
...
def get_session_image(session_image_id: int, db: Session = Depends(get_db)):
    session_image = db.query(models.SessionImage)\
        .options(joinedload(models.SessionImage.image))\
        .get(session_image_id)
...
def list_session_images(db: Session = Depends(get_db)):
    return db.query(models.SessionImage)\
        .options(joinedload(models.SessionImage.image))\
        .all()
