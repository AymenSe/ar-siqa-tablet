import React, {
  useState, useEffect, useCallback
} from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import axios from "axios";

function TrainingSession() {
  const navigate = useNavigate();
  const [sessionId, setSessionId] = useState(null);
  const [images, setImages] = useState([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [ratingValue, setRatingValue] = useState("");

  /**
   * Create a "training" session and assign images to it.
   */
  const createTrainingSession = useCallback(async (subject_id) => {
    try {
      // 1. Create a training session
      const sessionRes = await axios.post("http://127.0.0.1:8000/sessions", {
        subject_id: parseInt(subject_id, 10),
        session_type: "training"
      });

      setSessionId(sessionRes.data.session_id);

      // 2. Assign images to the session (hard-coded example: [1, 2, 3, 4, 5])
      const imagesToAssign = [1, 2, 3, 4, 5];
      await axios.post(
        `http://127.0.0.1:8000/session-images/${sessionRes.data.session_id}/assign_images`,
        imagesToAssign
      );

      // 3. Load the session images
      loadTrainingImages(sessionRes.data.session_id);
    } catch (error) {
      console.error("Error in creating or assigning session:", error);
      alert("Could not create or assign images for training session");
    }
  }, []);

  /**
   * Fetch the session images (with embedded image info) for the given session.
   */
  const loadTrainingImages = async (sId) => {
    try {
      const res = await axios.get("http://127.0.0.1:8000/session-images", {
        params: { session_id: sId },
      });
      setImages(res.data);
      setCurrentIndex(0);
    } catch (error) {
      console.error(error);
      alert("Could not load training images");
    }
  };

  /**
   * On mount: get subject_id from localStorage, create a training session.
   */
  useEffect(() => {
    const subject_id = localStorage.getItem("subject_id");
    if (!subject_id) {
      navigate("/register");
      return;
    }
    createTrainingSession(subject_id);
  }, [navigate, createTrainingSession]);

  /**
   * Submit a rating for the current image.
   */
  const handleRatingSubmit = async () => {
    if (currentIndex >= images.length) return;

    const sessionImage = images[currentIndex];
    try {
      await axios.post("http://127.0.0.1:8000/ratings", {
        session_image_id: sessionImage.session_image_id,
        question_id: 1,
        rating_value: parseFloat(ratingValue),
        text_answer: null,
        response_time: 2.5,
      });
      // Move to the next image
      setRatingValue("");
      setCurrentIndex((prev) => prev + 1);
    } catch (error) {
      console.error(error);
      alert("Failed to submit rating");
    }
  };

  /**
   * Optional "Next" or "Skip" button to skip rating or move on.
   */
  const handleNext = () => {
    if (currentIndex + 1 >= images.length) {
      // done with training => go to break
      navigate("/break");
    } else {
      setCurrentIndex((prev) => prev + 1);
    }
  };

  // Render states
  if (!sessionId) {
    return (
      <div className="p-8 text-gray-700">
        Loading Training Session...
      </div>
    );
  }

  if (!images.length) {
    return (
      <div className="p-8 text-gray-700">
        No training images assigned
      </div>
    );
  }

  if (currentIndex >= images.length) {
    return (
      <motion.div
        className="p-8 text-center"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <h2 className="text-2xl font-bold mb-4">Training Completed!</h2>
        <button
          onClick={() => navigate("/break")}
          className="px-6 py-2 bg-purple-500 text-white font-semibold rounded-2xl shadow-md hover:bg-purple-600 transition-all"
        >
          Proceed to Break
        </button>
      </motion.div>
    );
  }

  // Show current image
  const currentImage = images[currentIndex];
  const imageUrl = `http://127.0.0.1:8000/${currentImage.image?.file_path || ""}`;

  return (
    <motion.div
      className="p-8"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
    >
      <h2 className="text-2xl font-bold mb-2">Training Session</h2>
      <p className="mb-4">
        Image {currentIndex + 1} of {images.length}
      </p>
      <img
        src={imageUrl}
        alt={currentImage.image?.file_name || "training_image"}
        className="max-w-md mx-auto mb-4 rounded-2xl shadow-lg"
      />
      <div className="flex items-center justify-center mb-4">
        <label className="mr-2">Rating (1-5): </label>
        <input
          type="number"
          min="1"
          max="5"
          value={ratingValue}
          onChange={(e) => setRatingValue(e.target.value)}
          className="border border-gray-300 rounded-lg p-2 w-20 mr-2 text-center"
        />
        <button
          onClick={handleRatingSubmit}
          className="px-4 py-2 bg-green-500 text-white font-semibold rounded-2xl shadow-md hover:bg-green-600 transition-all"
        >
          Submit Rating
        </button>
      </div>
      <div className="flex justify-center">
        <button
          onClick={handleNext}
          className="px-4 py-2 bg-gray-300 text-gray-700 font-semibold rounded-2xl shadow-md hover:bg-gray-400 transition-all"
        >
          Skip/Next Image
        </button>
      </div>
    </motion.div>
  );
}

export default TrainingSession;
